	...
	<div class="container" id="container">
    	<div class="header">
            ...
            <div class="sitename" style="float:right; text-align:right">
                ...
                <h4>जिल्ला : <?=$district['district_name'];?></h4>
            	...
            	<tr>
            		<td>
						...
						if($typeId==CROP or $typeId==CROPCUTTING)
							$record=$program->getTableDataByTypeAndFiscalYearAndCropListAndUserId($typeId,$fiscalYear,$_POST['cropName'],$_GET['userId']);
						else if($typeId==MONTHLYREPORTING)
						  $record=$program->getTableDataByTypeAndFiscalYearAndCropListAndMonthAndUserId($typeId,$fiscalYear,$_POST['cropName'],$_POST['month'],
							$_GET['userId']);
						else if($typeId==PRICE)
							$record=$program->getTableDataByTypeAndFiscalYearAndPriceTypeAndMonthAndUserId($typeId,$fiscalYear,$_POST['priceType'],
							$_POST['month'],$_GET['userId']);
						else if($typeId==SUBSIDI)
						$record=$program->getTableDataByTypeAndFiscalYearAndProgramNameAndUserId($typeId,$fiscalYear,$_POST['programName'],$_GET['userId']);
						else
							$record=$program->getTableDataByTypeAndFiscalYearAndUserId($typeId,$fiscalYear,$_GET['userId']);
						}
						?>
                    	<? include($title['table_name']."dp.php");?>
            		</td>
          		</tr>
        	</table>
        </div>
