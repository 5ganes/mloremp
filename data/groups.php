...
class Groups
{
	function save($id, $name, $namenp, $urlname, $type, $parentId, $linkType, $shortcontents, $shortcontentsnp, $contents, $contentsnp, $weight, $pageTitle, $pageKeyword, $featured, $display)
	{
		global $conn;
		...
		$shortcontentsnp = cleanQuery($shortcontentsnp);
		$contentsnp = cleanQuery($contentsnp);
		...
		$sql = "UPDATE groups
						SET
							...
							contents = '$contents',
							shortcontentsnp = '$shortcontentsnp',
	...
	function saveListSub($id, $name, $namenp, $urlname, $parentId, $shortcontents, $shortcontentsnp, $contents, $contentsnp, $featured, $weight)
	{
		global $conn;
	...
	function saveProgramType($id, $program_name, $table_name, $publish, $weight)
	{
		global $conn;
		
		$id = cleanQuery($id);
		$program_name = cleanQuery($program_name);
		$table_name = cleanQuery($table_name);
		$publish = cleanQuery($publish);
		$weight = cleanQuery($weight);
		if($id > 0)
		...
		$sql = "INSERT INTO programtype SET program_name = '$program_name', table_name = '$table_name', publish='$publish', weight='$weight'";
		//echo $sql; die();
		$conn->exec($sql);
		if($id > 0)
			return $conn -> affRows();
	...
	//for Unit
	function saveUnit($id, $name, $category, $publish, $weight)
	{
		global $conn;
		...
		$sql = "INSERT INTO unit SET name = '$name', category = '$category', publish='$publish', weight='$weight'";
		$conn->exec($sql);
		if($id > 0)
			return $conn -> affRows();
		return $conn->insertId();
	...
	function getUnitLastWeight()
	{
		global $conn;
		$sql = "SElECT weight FROM unit ORDER BY weight DESC LIMIT 1";
		$result = $conn->exec($sql);
		$numRows = $conn -> numRows($result);
		if($numRows > 0)
		{
	...
	function getUnitByCategory($category)
	{
		global $conn;
		$category = cleanQuery($category);
		$sql = "SElECT * FROM unit WHERE category = '$category' and publish='Yes' order by weight";
		$result = $conn->exec($sql);
		return $result;
	}
