...
if($typeId==DISTRICT){
	...
	while ($row = mysql_fetch_array($record))
	{
		...
		$itu=$program->getUnitById($row['irrigationTimeUnit']); $itunit[]=$itu['name'];
	};
...
else if($typeId==MONTHLYREPORTING)
{
	...
	while ($row = mysql_fetch_array($record))
	{
		...
		$compen[]=$row['compensation']; $rem[]=$row['remarks'];
	};
	fputcsv($output, array('Fiscal Year','Insurance Holder','Insurance Company','Crop Name','Crop Area(H)','Insurance Amount(Rs)','Compensation','Remarks'));
	for($c=0;$c<count($fYear);$c++)
	{
		fputcsv($output, array($fYear[$c],$insName[$c],$insCompany[$c],$cName[$c],$cArea[$c],$iAmount[$c],$compen[$c],$rem[$c]));	
	}	
}
	...
	fputcsv($output, array('Fiscal Year','Farmer Name','VDC/Municipality','Ward No','Lake Type','Lake Number','Lake Area(H)','Fish Siecies','Production(Ton)'));
	for($c=0;$c<count($fYear);$c++)
	{
